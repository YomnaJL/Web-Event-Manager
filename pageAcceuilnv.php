...
// Vérifier si l'utilisateur est connecté (vous pouvez adapter cela selon votre logique d'authentification)
if (isset($_SESSION['user_id'])) {
    // Récupérer l'ID de l'utilisateur connecté
    $user_id = $_SESSION['user_id'];

    // Requête pour récupérer le nom de l'utilisateur depuis la table signuputi
    $sql = "SELECT nom FROM signuputi WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $user_id); // "i" pour int
    $stmt->execute();
...
<body>
    <nav class="navbar">
        ...
        <ul class="nav-links">
            ...
            <li><a href="recuperer_evenements.php">Evenements</a></li>
            <li><a href="mes_inscriptions.php">mes_clients</a></li>
        ...
        if (isset($username)) {
            echo "<div class='user-info'>Bonjour, $username</div>";
        }
        ?>
<a href="pageAcceuil.php" id="connect-btn" class="btn-inscription" onclick="confirmDeconnexion(event)">Se déconnecter</a>
...
<!-- Conteneur dynamique pour afficher verification.html -->
    ...
    <button onclick="window.location.href='temoignage.html';">Écrire un avis</button>
...
<script>
     function confirmDeconnexion(event) {
        ...
        const isConfirmed = confirm("Êtes-vous sûr de vouloir vous déconnecter ?");
        
        // Si l'utilisateur confirme, redirige vers la page de déconnexion
        if (isConfirmed) {
            window.location.href = event.target.href; // Redirige vers la page de déconnexion
        }
    }
